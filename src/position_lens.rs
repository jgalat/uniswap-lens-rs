...
use crate::{
    ...
    bindings::{
        ...
        ephemeralgetpositionv4::{EphemeralGetPositionV4, EphemeralGetPositionV4::getPositionCall as getPositionV4Call},
        ephemeralgetpositionsv4::{EphemeralGetPositionsV4, EphemeralGetPositionsV4::getPositionsCall as getPositionsV4Call},
    },
...
where
    ...
    P: Provider<N>,
{
    let deploy_builder = EphemeralGetPosition::deploy_builder(provider,amm.into(), npm, token_id);
    call_ephemeral_contract!(deploy_builder, getPositionCall, block_id)
}

pub async fn get_position_details_v4<N, P>(
    position_manager: Address,
    token_id: U256,
    provider: P,
    block_id: Option<BlockId>,
) -> Result<EphemeralGetPositionV4::PositionState, Error>
where
    N: Network,
    P: Provider<N>,
{
    let deploy_builder = EphemeralGetPositionV4::deploy_builder(provider,position_manager, token_id);
    call_ephemeral_contract!(deploy_builder, getPositionV4Call, block_id)
}
...
where
    ...
    P: Provider<N>,
{
    let deploy_builder = EphemeralGetPositions::deploy_builder(provider,amm.into(), npm, token_ids);
    call_ephemeral_contract!(deploy_builder, getPositionsCall, block_id)
}

pub async fn get_positions_v4<N, P>(
    position_manager: Address,
    token_ids: Vec<U256>,
    provider: P,
    block_id: Option<BlockId>,
) -> Result<Vec<EphemeralGetPositionsV4::PositionState>, Error>
where
    N: Network,
    P: Provider<N>,
{
    let deploy_builder = EphemeralGetPositionsV4::deploy_builder(provider, position_manager, token_ids);
    call_ephemeral_contract!(deploy_builder, getPositionsV4Call, block_id)
}
...
where
    ...
    P: Provider<N>,
{
    let deploy_builder = EphemeralAllPositionsByOwner::deploy_builder(provider,amm.into(), npm, owner);
...
mod tests {
    ...
    async fn verify_position_details(
            ...
            positions.into_iter().enumerate()
        {
            ...
            assert_eq!(position.feeOrTickSpacing, alt_positions[i].fee);
    ...
    async fn test_get_all_positions_by_owner() {
        ...
        let owner = npm
            ...
            .unwrap();
        let positions =
            get_all_positions_by_owner(Dex::UniswapV3,NPM_ADDRESS, owner, provider, Some(BLOCK_NUMBER))
                .await
                .unwrap();
